...
void
draw_line(SDL_Renderer *renderer, int x1, int y1, int x2, int y2)
{
    SDL_RenderLine(renderer, x1, y1, x2, y2);
}

void
draw_circle_outline(SDL_Renderer *renderer, int center_x, int center_y, int radius)
{
    int x = 0;
    int y = radius;
    int d = 3 - 2 * radius;
...
void
draw_hand(SDL_Renderer *renderer, int center_x, int center_y, double angle, int length, int thickness)
{
...
void
draw_hour_markers(SDL_Renderer *renderer, int center_x, int center_y, int radius)
{
    for (int hour = 0; hour < 12; hour++) {
        double angle = hour * 30.0 * M_PI / 180.0;
        int outer_x = center_x + (int)(radius * sin(angle));
        int outer_y = center_y - (int)(radius * cos(angle));
        int inner_x = center_x + (int)((radius - 20) * sin(angle));
        int inner_y = center_y - (int)((radius - 20) * cos(angle));
...
void
get_current_time(int *hours, int *minutes, int *seconds)
{
    time_t raw_time;
    struct tm *time_info;
...
render_clock(Clock *clock)
{
    ...
    draw_circle_outline(clock->renderer, CENTER_X, CENTER_Y, CLOCK_RADIUS);
    draw_hour_markers(clock->renderer, CENTER_X, CENTER_Y, CLOCK_RADIUS);
    ...
    draw_hand(clock->renderer, CENTER_X, CENTER_Y, hour_angle, 120, 6);
    draw_hand(clock->renderer, CENTER_X, CENTER_Y, minute_angle, 180, 4);
    
    SDL_SetRenderDrawColor(clock->renderer, 255, 0, 0, 255);
    draw_hand(clock->renderer, CENTER_X, CENTER_Y, second_angle, 200, 2);
    
    SDL_SetRenderDrawColor(clock->renderer, 255, 255, 255, 255);
    draw_circle_outline(clock->renderer, CENTER_X, CENTER_Y, 8);
...
init_clock(Clock *clock)
{
    ...
    clock->window = SDL_CreateWindow("Analogue Clock",
                                    WINDOW_WIDTH,
                                    WINDOW_HEIGHT,
                                    0);
